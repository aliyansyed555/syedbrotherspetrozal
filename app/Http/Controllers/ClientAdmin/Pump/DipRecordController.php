...
class DipRecordController extends Controller
{
    private $user;
    private $company;
    ...
    function index(Request $request)
    {
        ...
        // dd($tanks);
        return view('client_admin.pump.dip-record', compact(['pump_id', 'tanks']));
    }
    ...
    function get_all(Request $request)
    {
        ...
        $tanks = $pump->tanks;
        // $stocks = DipRecord::whereIn('tank_id', $tanks->pluck('id'))->get();
        $stocks = DipRecord::select('dip_records.*', 'tanks.name as tank_name')
    ...
    public function create(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'reading_in_mm.required' => 'The reading in mm field is required.',
            'reading_in_mm.numeric' => 'The reading in mm must be a valid number.',
            'reading_in_mm.regex' => 'The reading in mm can have up to 11 digits before the decimal and 2 digits after.',
        ...
        DipRecord::create([
            'reading_in_mm' => $validatedData['reading_in_mm'],
    ...
    public function update(Request $request, $pump_id)
    {
        ...
        $validatedData = $request->validate([
            'id' => 'required|exists:dip_records,id',
            'reading_in_mm' => [
                'required',
                'numeric',
                'regex:/^\d{1,11}(\.\d{1,2})?$/',
        ...
        try {
            ...
            $tankStock = DipRecord::whereHas('tank', function ($query) use ($pump) {
    ...
    public function delete($pump_id, $id)
    {
        ...
        try {
            $pricing = DipRecord::findOrFail($id);
            $pricing->delete();
